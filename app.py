from PyQt5.QtWidgets import QApplication, QFileDialog, QLabel, QVBoxLayout, QWidget, QLineEdit, QPushButton, QFormLayout, QCheckBox, QPlainTextEdit
...
class MainWindow(QWidget):
    ...
    def iniUI(self):
        ...
        run_layout.addWidget(self.button3, alignment=Qt.AlignHCenter)
        run_layout.addWidget(self.text, alignment=Qt.AlignHCenter)
    ...
    def measure_length(self):
        self.text.clear()
        if self.input_path == "":
            self.message("Please select the image input folder!!")
        if self.output_path == "":
            self.message("Please select the output folder!!")

        if self.input_path != "" and self.output_path != "":
            self.message("Input folder: " + self.input_path)
            self.message("Ouput folder: " + self.output_path)
            self.message("Running started......Please wait......")
            self.text.show()
            try:
                run_calc(self.input_path, self.output_path, self.minlength_val, self.keep_edges)
                self.message("Completed!")
            except:
                self.message("Errors out!!!!")
...
def get_nearby(to_resolve_starts, to_resolve_ends, anchor_path_coord_end):
    starts_dist = [(x[0] - anchor_path_coord_end[0]) ** 2 + (x[1] - anchor_path_coord_end[1]) ** 2 for x in
                   to_resolve_starts]
    end_dist = [(x[0] - anchor_path_coord_end[0]) ** 2 + (x[1] - anchor_path_coord_end[1]) ** 2 for x in
                to_resolve_ends]
...
def search_next_path(anchor_path_coord_end, to_resolve_df, to_resolve_starts, to_resolve_ends, anchor_direct,
                     checked_single_start, new_g, delta, angle_cutoff):
    ...
    last_start_0, last_start_1, last_end_0, last_end_1 = to_resolve_df.loc[last_path, "image-coord-src-0"], \
                                                         to_resolve_df.loc[last_path, "image-coord-src-1"], \
                                                         to_resolve_df.loc[last_path, "image-coord-dst-0"], \
                                                         to_resolve_df.loc[last_path, "image-coord-dst-1"]
    ...
    if (to_review.shape[0] > 0):
        ...
        for rr in range(to_review.shape[0]):
            ...
            direction = find_mid_direction(rr_index, anchor_path_coord_end, to_resolve_df, to_resolve_starts,
                                           to_resolve_ends)
            if (len(review_path_coord) >= delta):
                review_path_coord_start, review_path_coord_start_delta = get_path_node(direction, rr_index, delta,
                                                                                       to_review, review_path_coord,
                                                                                       node_type="start")
...
def find_mid_direction(to_extend, last_end, to_resolve_df, to_resolve_starts, to_resolve_ends):
    ...
    to_extend_src = [to_resolve_df.loc[to_extend, "image-coord-src-0"],
                     to_resolve_df.loc[to_extend, "image-coord-src-1"]]
    to_extend_dst = [to_resolve_df.loc[to_extend, "image-coord-dst-0"],
                     to_resolve_df.loc[to_extend, "image-coord-dst-1"]]
...
def resolve_path(to_resolve, new_stats, new_g, delta, angle_cutoff):
    ...
    for ri in to_resolve:
        ...
        to_resolve_tips = to_resolve_df[
            (to_resolve_df["branch-type"] == 1) & (to_resolve_df["branch-distance"] >= delta)].index
        ...
        for ai in to_resolve_tips:
            if not (ai in checked_start):
                ...
                anchor_path_coord_end, anchor_path_coord_end_delta = get_path_node(direction, ai, delta, to_resolve_df,
                                                                                   anchor_path_coord)
                anchor_direct = angle(anchor_path_coord_end_delta, anchor_path_coord_end)
                # print("anchor direct " + str(anchor_direct))
                while to_extend != "":
                    to_extend, path_type, angle_diff = search_next_path(anchor_path_coord_end, to_resolve_df,
                                                                        to_resolve_starts, to_resolve_ends,
                                                                        anchor_direct, checked_single_start, new_g,
                                                                        delta, angle_cutoff)
                    ...
                    if (to_extend != ""):
                        ...
                        direction = find_mid_direction(to_extend, anchor_path_coord_end, to_resolve_df,
                                                       to_resolve_starts, to_resolve_ends)
                        # print(direction)
                        anchor_path_coord = new_g.path_coordinates(to_extend)
                        anchor_path_coord_end, anchor_path_coord_end_delta = get_path_node(direction, to_extend, delta,
                                                                                           to_resolve_df,
                                                                                           anchor_path_coord)
    ...
    for ind_p in path_decreasing:
        if new_stats.loc[ind_p, "branch-distance"] > 40:
            ...
            for path in path_list:
                if (not (dedup) and (ind_p in path[0])) or (
                        dedup and (ind_p in path[0]) and not (path in checked_path_list)):
...
def output_path_plot(input_image_name, output_image_base, in_file_dir, out_file_dir, min_length, include_edge, delta=10,
                     angle_cutoff=80):
    ...
    path_to_plot, length_collection = collect_length(resovled_path_list_uniq, new_stats, image_shape, min_length,
                                                     include_edge)
    ...
    length_dic = {"sample_id": image_number, "image_type": image_type,
                  "hair_id": [i for i in range(len(length_collection))], "length": length_collection}
    return length_dic


def run_calc( in_file_dir, out_file_dir, min_length, include_edge):
    print("Calculating.....")
    ...
    for image in all_images:
        ...
        length_dic = output_path_plot(image_name, output_image_base, in_file_dir, out_file_dir, min_length,
                                      include_edge, delta=15, angle_cutoff=80)
